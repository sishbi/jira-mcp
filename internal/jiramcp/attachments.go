...
// validateTextAttachment enforces the text-only policy on the (declared mime,
// body bytes) pair. Two checks must pass:
//
...
//
// An empty declared mime ("upload, mime not yet known") is accepted as a hint
// to rely on byte sniffing alone.
func validateTextAttachment(declaredMIME string, body []byte) error {
	if declaredMIME != "" && !isAllowedTextMime(declaredMIME) {
